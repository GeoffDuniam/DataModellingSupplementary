...
def get_data(raw_vectors_df, extragalactic=None, use_specz=False):
    ...
    for key in object_vectors.keys():
        ...
        sample['target'] = int(i.get('target')) # We use this if the target construct has been cretaed in the table
        ...
        fwd_int=np.array( j[0][4], dtype='float32')
        bwd_int=np.array( j[0][5], dtype='float32')
        source_wavelength=np.array( j[0][6], dtype='int32') #.reshape(c,) #         
        received_wavelength=np.array( j[0][7], dtype='int32') #.reshape(c,) # 

        # Calculations already in data model
        #mjd -= mjd[0]
        ...
        sample['hist'][:,4] = fwd_int
        sample['hist'][:,5] = bwd_int
...
if __name__ == '__main__':
    ...
    logger.info("Beginning Fourth pivot model test at {}".format(str(start_elapsed)) )
    ...
    raw_vectorsDF=sc.sql("select * from training_set_raw_vectors_unpadded_with_calcs")
