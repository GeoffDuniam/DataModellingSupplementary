...
def get_data(mjd_df, passband_df, flux_df, flux_err_df, detected_df, meta_df, extragalactic=None, use_specz=False):
    ...
    for g in groups:
        ...
        band = np.array(passband_df.loc[passband_df['object_id'] == id].dropna(axis='columns') , \
                        dtype='int32')[:,idx_IN_columns].reshape(c-1,)
        flux = np.array(flux_df.loc[flux_df['object_id'] == id].dropna(axis='columns') , \
                        dtype='float32')[:,idx_IN_columns].reshape(c-1,)
        flux_err = np.array(flux_err_df.loc[flux_err_df['object_id'] == id].dropna(axis='columns') , \
                            dtype='float32')[:,idx_IN_columns].reshape(c-1,)
        detected = np.array(detected_df.loc[detected_df['object_id'] == id].dropna(axis='columns') , \
                            dtype='float32')[:,idx_IN_columns].reshape(c-1,)
...
if __name__ == '__main__':
    ...
    logger.info("Beginning First pivot model test at {}".format(str(start_elapsed)) )
    ...
    train_mjd_data=sc.sql("select * from full_mjd_pivot").toPandas()
    train_passband_data=sc.sql("select * from full_passband_pivot").toPandas()
    train_flux_data=sc.sql("select * from full_flux_pivot").toPandas()
    train_flux_err_data=sc.sql("select * from full_flux_err_pivot").toPandas()
    train_detected_data=sc.sql("select * from full_detected_pivot").toPandas()
    

    logger.info('Loading the training meta data from hive')
    train_meta=sc.sql("select * from training_set_metadata").toPandas()
